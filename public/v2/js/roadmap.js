...
async function initializeRoadmap() {
    try {
        ...
        if (kanbanContainer) {
            // Don't replace the whole board, just show error in columns or a global error
            // But the original code replaced the container content.
            // Let's just log it for now or show a toast if we had one.
            // Or better, set error message in each column.
            Object.values(columnMap).forEach(colId => {
                const col = document.getElementById(colId);
                if (col) col.innerHTML = '<p class="error-message">Failed to load.</p>';
            });
        }
    }
}
